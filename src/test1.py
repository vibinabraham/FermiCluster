    ...
    for fock_ri, fock_r in enumerate(ci_vector.data):
        ...
        for config_li, config_l in enumerate(configs_l):
            for config_ri, config_r in enumerate(configs_r):        
                idx_l = config_li + shift_l 
                idx_r = config_ri + shift_r
                ...
                for term in terms:
                    ...
                    H[idx_l,idx_r] += me
                    print(" %4i %4i = %12.8f"%(idx_l,idx_r,me),fock_l,fock_r,":",config_l,config_r)
...
#            for config_li, config_l in enumerate(configs_l):
#                for config_ri, config_r in enumerate(configs_r):
#                    idx_l = config_li + fock_li*len(config_l)
#                    idx_r = config_ri + fock_ri*len(config_r)
#                    for term in terms:
#                        H[idx_l,idx_r] += term.matrix_element(fock_l,config_l,fock_r,config_r)
